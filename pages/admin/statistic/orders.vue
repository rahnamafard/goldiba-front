...
export default {
  ...
  methods: {
    ...
    async loadDiagrams() {
      ...
      try {
        ...
        if (response.data.data.length > 0) {
          ...
          labels = results.map(
            ...
            // result => this.$jalaliDateTime(result.period, "YYYY/M/DTHH:mm").date
          );
      ...
      PE_dataset.data = await this.goGetData("PE", labels);
      AP_dataset.data = await this.goGetData("AP", labels);
      EX_dataset.data = await this.goGetData("EX", labels);
      ...
      let datasets = [all_dataset, PE_dataset, AP_dataset, EX_dataset];
